...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".jigar_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".jigar_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using JIGAR profile")
                jigar = str(Path.home() / ".jigar_profiles" / "firefox_jigar")
                Path(jigar).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(jigar, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".jigar_profiles" / "safari")
        ...
        jigar_profile = str(Path.home() / ".jigar_profiles" / self.browser_name)
        Path(jigar_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with JIGAR profile: {jigar_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jigar_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with JIGAR profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "jigar_screenshot.png")
