"""
dashboard/server.py — JIGAR Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "JIGAR Uploads",
        Path.home() / "Documents" / "JIGAR Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'JIGAR-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"JIGAR Dashboard Port {port}"
        prog_rule  = "JIGAR Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="jigar_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until JIGAR is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "jigar.key").exists() and (certs / "jigar.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in JIGAR to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('jigar_token','{tok}');
  sessionStorage.setItem('jigar_key','{key}');
  localStorage.setItem('jigar_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to JIGAR…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "jigar.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "jigar.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in JIGAR UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
